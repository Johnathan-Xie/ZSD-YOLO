...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', annot_folder='labels',
                     raw=False):
    ...
    with torch_distributed_zero_first(rank):
        dataset = loader_type(path, imgsz, batch_size,
...
def img2label_paths(img_paths, image_folder='images', annot_folder='labels', suffix='txt'):
    # Define label paths as a function of image paths
    sa, sb = os.sep + image_folder + os.sep, os.sep + annot_folder + os.sep  # /images/, /labels/ substrings
    return [suffix.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]


class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',
                 annot_folder='labels', raw=False):
        self.img_size = img_size
        ...
        self.label_files = img2label_paths(cache.keys(), self.image_folder, self.annot_folder)  # updatea
        if single_cls:
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size and (not self.raw):  # normalized xywh to pixel xyxy format
...
class LoadZSD(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='', cross_check_labels=True,
                annot_folder='labels', raw=False):
        self.img_size=img_size
        self.augment = augment
        self.hyp = hyp
        ...
        if cross_check_labels:
            ...
            for name in tqdm(os.listdir(label_path)):
                try:
                    ...
                    label_names.append(name.split('.')[0])
                    empty_files += len(image_embeddings) == 0
                except Exception as e:
        ...
        # Load labels and shapes.  If you don't have enough RAM ¯\_(ツ)_/¯
        self.labels = [torch.load(i).numpy() for i in tqdm(self.label_files)]
...
def extract_boxes(path='../coco128/'):  # from utils.datasets import *; extract_boxes('../coco128')
    ...
    for im_file in tqdm(files, total=n):
        if im_file.suffix[1:5] in img_formats:
            # image
