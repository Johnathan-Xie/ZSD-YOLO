...
def xyxy2xywhn(x, w=640, h=640, padw=0, padh=0):
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = ((x[:, 0] + x[:, 2]) / 2 - padw) / w
    y[:, 1] = ((x[:, 1] + x[:, 3]) / 2 - padh) / h
    y[:, 2] = ((x[:, 2] - x[:, 0])) / w
    y[:, 3] = ((x[:, 3] - x[:, 1])) / h
    return y
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    #print(xc.sum())
    #if zsd:
    #    xc = torch.logical_and(xc, prediction[:, 4] * prediction[:, 5:].max(dim=1)[0] > obj_conf_thresh)
    #print(xc.sum())
    # Checks
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if zsd:
            if len(eval_splits):
                new_x = torch.zeros(size=(x.shape[0] * len(eval_splits), x.shape[1] - 5)).cuda()
        ...
        if multi_label:
            ...
            #if zsd:
                #print(torch.zeros_like(x[i, j + 5, None]) + x[i, 4], x[i, j + 5, None])
                #print((torch.zeros_like(x[i, j + 5, None]) + x[i, 4]).shape, x[i, j + 5, None].shape)
            #    x = torch.cat((box[i], torch.zeros_like(x[i, j + 5, None]) + x[i, 4], j[:, None].float()), 1)
            #else:
        ...
        #print(x[0])
        #if conf_multiplier != None:
        #    x[:, 4] = x[:, 4] * torch.index_select(conf_multiplier, dim=0, index=x[:, 5].type(torch.cuda.LongTensor))
        #    print(x[0])
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket='', opt=None):
    ...
    for i, k in enumerate(hyp.keys()):
        hyp[k] = float(x[0, i + (11 if opt.zsd else 7)])
    with open(yaml_file, 'w') as f:
        results = tuple(x[0, :(11 if opt.zsd else 7)])
        c = '%10.4g' * len(results) % results  # results (P, R, mAP@0.5, mAP@0.5:0.95, val_losses x 3)
